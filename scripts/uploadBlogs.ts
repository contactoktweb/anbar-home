...
async function main() {
  ...
  const b7Img1 = await uploadImage(path.join(b7Path, 'como-elegir-jarrones-decorativos-portada.webp'), 'como-elegir-jarrones-decorativos-portada.webp')
  const b7Img2 = await uploadImage(path.join(b7Path, 'jarrones-decorativos-segun-tamano.webp'), 'jarrones-decorativos-segun-tamano.webp')
  const b7Img3 = await uploadImage(path.join(b7Path, 'acabados-texturas-jarrones-decorativos.webp'), 'acabados-texturas-jarrones-decorativos.webp')
  const b7Img4 = await uploadImage(path.join(b7Path, 'jarron-con-ramas-decorativas.webp'), 'jarron-con-ramas-decorativas.webp')
  const b7Img5 = await uploadImage(path.join(b7Path, 'jarrones-para-sala-comedor-recibidor.webp'), 'jarrones-para-sala-comedor-recibidor.webp')

  const blog7Body = [
    block('Saber cómo elegir jarrones decorativos ayuda a evitar dos problemas frecuentes: comprar una pieza que se ve pequeña frente al mueble o elegir una composición que ocupa demasiado espacio. Antes de decidir por el color o la forma, conviene revisar las medidas, la ubicación y el efecto visual que se busca.'),
    block('El criterio cambia según el ambiente. Una mesa auxiliar necesita una pieza distinta a la de una consola amplia, un recibidor, una oficina o un lobby. Esta guía reúne pautas prácticas para comparar tamaños, acabados y formas de uso sin depender de una tendencia específica.'),
    ...
    block('Mide el ancho, el fondo y la altura de la superficie donde irá el jarrón. La base debe quedar completamente apoyada y debe existir espacio libre alrededor. Si el jarrón llevará flores o ramas, calcula también la altura total de la composición para comprobar que no cubra un cuadro, un espejo, una ventana o una señal dentro de un espacio comercial.'),
    block('La vista frontal no es suficiente. Revisa el jarrón desde el recorrido habitual del ambiente y confirma que no interfiera con puertas, zonas de paso o elementos que se usan a diario.'),

    block('Jarrones pequeños', 'h3'),
    block('Funcionan bien en mesas auxiliares, mesas de noche, escritorios y repisas. Pueden acompañarse con libros o cajas decorativas, siempre que mantengan una diferencia visible de altura. En superficies estrechas es preferible una pieza estable y con una base que no quede al borde.'),

    block('Jarrones medianos', 'h3'),
    block('Son una opción útil para consolas, aparadores y mesas de centro amplias. Un diseño con suficiente presencia puede utilizarse sin flores. Si se combina con otros objetos, debe conservar un área libre que permita apreciar su silueta.'),

    block('Jarrones grandes', 'h3'),
    block('Pueden ubicarse sobre muebles profundos o directamente en el piso. Antes de instalarlos, verifica su estabilidad y el espacio de circulación. En recibidores, oficinas, hoteles y locales comerciales también conviene observar la pieza desde la entrada para revisar su proporción frente al muro y al mobiliario.'),
    ...
    block('Un jarrón escultórico puede funcionar como objeto decorativo sin relleno. Esta opción permite apreciar la forma completa y resulta útil cuando el entorno ya tiene varios patrones o accesorios.'),
    block('Si se utilizan ramas, su altura debe mantener una relación visual con el recipiente y el techo. Distribúyelas sin ocultar por completo la silueta del jarrón. Frente a un espejo o una obra de arte, comprueba que el follaje no cubra el punto focal ni genere una masa visual difícil de leer.'),
    block('Para mesas de comedor, la altura total también debe permitir la conversación entre las personas sentadas. En consolas y aparadores, puede utilizarse una composición más alta si no interfiere con luminarias, cuadros o circulación.'),
    ...
    block('Busca un vínculo claro entre las piezas: color, acabado, forma o lenguaje visual. No necesitan ser idénticas. Una combinación se entiende mejor cuando existe una pieza principal y las demás funcionan como apoyo.'),
    block('Usa alturas distintas y acerca las bases para que el conjunto se lea como una sola composición. Evita distribuir los jarrones a distancias iguales o formar una fila rígida. Adelantar ligeramente una pieza puede aportar profundidad sin ocupar más superficie.'),
    block('Si los jarrones tienen relieves marcados, reduce la cantidad de accesorios cercanos. Si son más sencillos, pueden acompañarse con una bandeja, libros u otro objeto de menor escala. La selección debe dejar zonas de descanso visual.'),

    block('Qué jarrón elegir para cada espacio', 'h2'),
    block('Sala', 'h3'),
    block('En una mesa de centro, el jarrón debe dejar una parte útil de la superficie. Sobre una consola o un mueble de televisión puede tener mayor altura, siempre que no cubra la pantalla, el arte o el espejo. Para ampliar este criterio, consulta la guía de Anbar Home sobre cómo decorar una sala sin recargarla.', 'normal', [
    ...
    block('Comedor', 'h3'),
    block('Si el jarrón permanece en la mesa durante las comidas, revisa que no bloquee la conversación. Una pieza más alta puede funcionar en un aparador cercano, donde tenga suficiente fondo y no interfiera con el servicio.'),

    block('Recibidor', 'h3'),
    block('Elige una pieza visible desde la entrada y proporcionada con la consola. Revisa la apertura de la puerta, el paso de las personas y la distancia frente al espejo. Una composición alta puede dirigir la mirada, pero necesita aire alrededor.'),

    block('Oficina, hotel o espacio comercial', 'h3'),
    block('En proyectos profesionales, la escala debe evaluarse desde varios puntos de circulación. En una recepción o lobby, un jarrón puede ayudar a organizar el punto focal. En una sala de reuniones o un escritorio compartido, debe conservar la visibilidad y el área de trabajo.'),

    block('Repisas y piso', 'h3'),
    block('Para repisas, utiliza formatos que permitan dejar espacio libre en cada nivel. En el piso, escoge una base estable y ubica la pieza fuera de recorridos, puertas y zonas de limpieza frecuente.'),
    ...
    block('No necesita repetir exactamente su color. Puede relacionarse con cojines, cortinas, arte, mesas o acabados del ambiente. También puede crear contraste si ese tono aparece en otro punto de la composición.'),

    block('¿Se pueden mezclar jarrones con acabados diferentes?', 'h3'),
    block('Sí, cuando existe un vínculo visible entre las piezas. Puede ser la forma, la paleta o la proporción. Conviene elegir una pieza principal y limitar la cantidad de materiales que compiten en la misma superficie.'),

    block('¿Cómo saber si un jarrón es demasiado grande?', 'h3'),
    block('Observa si ocupa casi todo el fondo del mueble, cubre elementos importantes o dificulta el uso de la superficie. También debe verse completo desde la distancia habitual y conservar espacio libre a su alrededor.'),

    block('¿Cuántos jarrones se pueden colocar juntos?', 'h3'),
    block('Depende del tamaño de la superficie. Dos o tres piezas con alturas diferentes suelen permitir una composición clara, pero el criterio principal es que las bases estén seguras y que todavía exista espacio libre.'),

    block('Elige con las medidas del espacio', 'h2'),
    block('La decisión debe comenzar por el lugar donde irá la pieza. Conserva las medidas, define si utilizarás ramas o flores y compara la escala desde el recorrido habitual del ambiente. Puedes explorar la colección de jarrones escultóricos de Anbar Home para revisar opciones disponibles y contrastarlas con tu espacio.', 'normal', [
      ...
      { text: 'jarrones escultóricos de Anbar Home', link: 'https://www.anbarhome.co/category/jarrones-escultoricos' },
  ...
  await client.createOrReplace({
    ...
    metaDescription: 'Aprende cómo elegir jarrones decorativos según el tamaño, acabado y espacio. Ideas para salas, comedores, recibidores, oficinas y más.',
  ...
  console.log('✓ Blog 7 Saved successfully')

  console.log('\n==========================================')
  console.log('ALL 7 BLOGS SUCCESSFULLY UPLOADED TO SANITY!')
  console.log('==========================================')
}
